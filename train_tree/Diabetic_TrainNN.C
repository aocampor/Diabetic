...
void Diabetic_TrainNN( TString myMethodList = "" )
{
  ...
  //factory->AddVariable( "Green/Red", 'F' );
  //factory->AddVariable( "Blue/Red", 'F' );
  factory->AddVariable( "Green*Blue/Red", 'F' );
  //factory->AddVariable( "(Green-Red)/Red", 'F' );
  ...
  TString fname = "../Image.root";
  TString fname1 = "../sig.root";
  ...
  TTree *signal     = (TTree*)input1->Get("Images");
  TTree *background = (TTree*)input->Get("Images");
  ...
  TCut mycuts = "drlevel > 0"; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
  TCut mycutb = "drlevel == 0"; // for example: TCut mycutb = "abs(var1)<0.5";
