...
FitMethod: "MC" [Optimisation Method]
Converger: "MINUIT" [FitMethod uses Converger to improve result]
# Default:
...
class ReadFDA_MCMT : public IClassifierReader {

 public:

   // constructor
   ReadFDA_MCMT( std::vector<std::string>& theInputVars ) 
      : IClassifierReader(),
        fClassName( "ReadFDA_MCMT" ),
        fNvars( 5 ),
        fIsNormalised( false )
   {      
...
inline void ReadFDA_MCMT::Initialize() 
{
   fParameter[0] = 0.499999519275559;
   fParameter[1] = -2.37853194586933;
   fParameter[2] = 1.53802998829633;
   fParameter[3] = 0.585315562784672;
   fParameter[4] = 4.84959369990975;
}

inline double ReadFDA_MCMT::GetMvaValue__( const std::vector<double>& inputValues ) const
{
...
// Clean up
inline void ReadFDA_MCMT::Clear() 
{
   // nothing to clear
}
   inline double ReadFDA_MCMT::GetMvaValue( const std::vector<double>& inputValues ) const
   {
