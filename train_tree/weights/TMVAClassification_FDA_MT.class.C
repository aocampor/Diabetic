...
FitMethod: "MINUIT" [Optimisation Method]
# Default:
...
class ReadFDA_MT : public IClassifierReader {

 public:

   // constructor
   ReadFDA_MT( std::vector<std::string>& theInputVars ) 
      : IClassifierReader(),
        fClassName( "ReadFDA_MT" ),
        fNvars( 5 ),
        fIsNormalised( false )
   {      
...
inline void ReadFDA_MT::Initialize() 
{
   fParameter[0] = 0.499807382904486;
   fParameter[1] = -0.200033301612006;
   fParameter[2] = -0.200033301612006;
   fParameter[3] = -0.200033301612006;
   fParameter[4] = -0.200033301612006;
}

inline double ReadFDA_MT::GetMvaValue__( const std::vector<double>& inputValues ) const
{
...
// Clean up
inline void ReadFDA_MT::Clear() 
{
   // nothing to clear
}
   inline double ReadFDA_MT::GetMvaValue( const std::vector<double>& inputValues ) const
   {
