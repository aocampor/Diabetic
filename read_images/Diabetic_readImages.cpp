...
#define __trainLabels 		"/media/idarraga/data/Diabetic_Retinopathy_Detection/trainLabels.csv"
...
void BuildNtuple(JSAMPLE * RGB, int width_i, int height_i, input_parameters ip) {
	...
	// Branches
	ip.roottree->Branch("Red", &colorR, "Red/D");
	ip.roottree->Branch("Green", &colorG, "Green/D");
	ip.roottree->Branch("Blue", &colorB, "Blue/D");
...
void InImageSelection(string fn, JSAMPLE * RGB, int width, int height) {
	...
	cout << "[INFO] Crop : " << x0 << "," << y0 << " --> " << x1 << "," << y1 << endl;
...
int main(int argc, char ** argv) {
	...
	if( ! DirectoryExists(inputp.output_dir) ) {
		cout << "[INFO] mkdir --> ip.output_dir" << endl;
...
void ProcessImages(vector<pair<string, int> > csvinfo, input_parameters ip) {
	...
	for( ; i != iE ; i++) {
		...
		ntuplefn += ip.fnbit;
		ntuplefn += ".root";
		cout << "[INFO] Creating ROOT file : " << ntuplefn << endl;
...
string LowerRes(string file, double lowrespercentage, string prefix, input_parameters ip) {
	...
	if ( stat(outfn.c_str(), &buffer) == 0 ) {
		cout << "[INFO] file : " << outfn << " already exists. Skip convert step." << endl;
		return outfn;
	}
...
vector<pair<string, int> > readCSVFile(string fn) {
	...
	cout << "[INFO] Read " << csv_info.size() << " entries from : " << fn << endl;
...
void DumpCSVInfo(vector<pair<string, int> > v) {
	...
	cout << "-- Dump CSV vector ---- first and last " << __lenghtprintcsv << " elements ------" << endl;
	for( ; i != iE ; i++) {
		...
		if(cntr == __lenghtprintcsv && dotsFlag) cout << " ... ";
...
bool DirectoryExists(string dir) {
	...
	if((dp = opendir(dir.c_str())) == NULL) {
		cout << "[INFO] Error trying to open directory : " << dir << endl;
		return false;
	}
...
static const int GetConcurrentThreads() {
	...
	cout << "[INFO] Concurrency setup :" << endl;
...
read_JPEG_file (char * filename, int & width, int & height) {
	...
	while (cinfo.output_scanline < cinfo.output_height) {
		...
		for ( int i = 0 ; i < row_stride ; i++) {
			...
			//cout << (unsigned int)buffer[0][i] << "," << (unsigned int)buffer[0][i+1] << "," << (unsigned int)buffer[0][i+2] << "  " ;
			gcntr++;
		}
